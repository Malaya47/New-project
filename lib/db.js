...
function loadDb() {
  ...
  try {
    ...
    if (!parsed.admins || !Array.isArray(parsed.admins) || parsed.admins.length === 0) {
      parsed.admins = defaultDb().admins;
      saveDb(parsed);
    }
...
function getCustomerByBagCode(bagCode) {
  const data = loadDb();
  return data.customers.find(
    (customer) => customer.bagCode === String(bagCode || "").trim()
  );
}
...
function getAdminByEmail(email) {
  ...
  return data.admins.find(
    (admin) => admin.email === String(email).trim().toLowerCase()
  );
}
...
function getAdminOverview() {
  const data = loadDb();
  const activeOrders = data.orders.filter((order) => order.status !== "completed").length;
  const revenue = data.orders.reduce((sum, order) => sum + Number(order.amount || 0), 0);

  return {
    stats: {
      customers: data.customers.length,
      orders: data.orders.length,
      revenue: new Intl.NumberFormat("de-CH", {
        style: "currency",
        currency: "CHF",
    ...
    statusOptions: ["scheduled", "picked_up", "processing", "returning", "completed"],
  };
}
